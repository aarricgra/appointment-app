...
export default function AppointmentInfo({route}) {
    ...
    const getAppointment = ()=>{
        GlobalApi.getAppoitmentById(route.params.id).then((res)=>setAppoitment(res.data.data[0].attributes))}
    ...
    if(appoitment!=null){ 
        GlobalApi.getServiceById(appoitment.idServicio.data.id).then((res2)=>setServicio(res2.data.data[0].attributes))
    }
    ...
    if(servicio!=null){
        return (
            <View style={{
              ...
              <Text style={{color:"white"}}>Servicio Duracion: {servicio.Duracion.split(":")[1]} min</Text>
